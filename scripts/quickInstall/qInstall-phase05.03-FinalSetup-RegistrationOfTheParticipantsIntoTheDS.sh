...
DSFIWAREHOL_TAG="phase05.step03"
STEP_DESCRIPTION="Addition of the service route to the Apisix with VC Authentication"
...
. scripts/manageAPI6Routes.sh insert -r ROUTE_PROVIDER_SERVICE_fiwaredsc_provider_local_authentication > /dev/null
...
if test "$RC" -eq 200; then
    ...
    if [ $? -eq 0 ]; then
        ...
        \t-Finally, the service is exposed just checking the authentication layer.
