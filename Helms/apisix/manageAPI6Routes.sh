...
# https://fiwaredsc-consumer.ita.es/
ROUTE_CONSUMER_WALLET_fiwaredsc_wallet_ita_es='{
  "uri": "/*",
  ...
  "upstream": {
    ...
    "scheme": "http",
    "nodes": {
      "wallet.consumer.svc.cluster.local:3000": 1
    }
  }
}'

curl -i -X POST -k https://$IP_APISIXCONTROL:9180/apisix/admin/routes -H "X-API-KEY:$ADMINTOKEN" \
-d "$ROUTE_CONSUMER_WALLET_fiwaredsc_wallet_ita_es"
# curl -i -X POST -k https://$IP_APISIXCONTROL:9180/apisix/admin/routes -H "X-API-KEY:$ADMINTOKEN" \
...
# Detele a route
# curl -i -X DELETE -k -H "X-API-KEY:$ADMINTOKEN" https://$IP_APISIXCONTROL:9180/apisix/admin/routes/00000000000000000241
